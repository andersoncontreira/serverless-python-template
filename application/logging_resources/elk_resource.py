"""
ELK Logging Resource Module for Application
Version: 1.0.0
"""
import datetime
import logging
...
class ELKHandler(logging.Handler):
    def __init__(self, es_client=None, **kwargs):
        super().__init__()
        ...
        if 'es_client' not in kwargs:
            try:
                from application.aws.opensearch import get_elasticsearch_client
                self.es_client = get_elasticsearch_client()
            except Exception as err:
                raise Exception('Module elasticsearch unavailable')
        else:
