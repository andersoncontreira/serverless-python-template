...
class ProductRepositoryTestCase(BaseComponentTestCase):
    ...
    def setUpClass(cls):
        ...
        connection = RedisHelper.get_connection()
        # connection = RedisHelper.get
        logger = get_logger()

        if cls.EXECUTE_FIXTURE:
            # delete all keys generated by the others tests
            repository = ProductRepository(redis_connection=connection)
            try:
                deleted = repository.delete(key='%s:%s' % ('product', 'fecfddd9-7cb8-413b-9de3-ec86de30a888'))
                if deleted:
                    logger.info('Product deleted: fecfddd9-7cb8-413b-9de3-ec86de30a888')
            except Exception as err:
                logger.info('Register not found in redis: {}'.format(err))
            try:
                deleted = repository.delete(key='%s:%s' % ('product', '8374b976-a74e-475c-b78c-39717468926c'))
                if deleted:
                    logger.info('Product deleted: 8374b976-a74e-475c-b78c-39717468926c')
            except Exception as err:
                logger.info('Register not found in redis: {}'.format(err))
