...
from application.decorators.events import SQSRecord, SQSEvent
...
def get_sqs_event_sample():
    with open(path.join(ROOT_DIR, 'tests/datasources/events/sqs/products/product-create-event.json')) as f:
        event_str = f.read()
    try:
